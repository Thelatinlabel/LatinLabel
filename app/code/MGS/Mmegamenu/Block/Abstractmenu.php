...
abstract class Abstractmenu extends \Magento\Framework\View\Element\Template implements \Magento\Framework\DataObject\IdentityInterface
{
	...
	public function getCategoryMenu($item) {
        ...
        if (count($subCatAccepp) > 0 || $item->getTopContent() != '' || $item->getBottomContent() != '') {
            $html.='<span class="toggle-menu"><a onclick="toggleEl(this,\'mobile-menu-' . $item->getId() . '-'. $item->getParentId() . '\')" href="javascript:void(0)" class=""><span class="fa fa-angle-down"></span></a></span>';
	...
	public function drawList($category, $item, $level = 1) {
        ...
        if ($level < $maxLevel) {
            ...
            if (!empty($htmlChildren)) {
                $html[] = '<span class="toggle-menu"><a onclick="toggleEl(this,\'mobile-menu-cat-' . $category->getId() . '-' . $item->getParentId() . '\')" href="javascript:void(0)" class=""><span class="fa fa-angle-down"></span></a></span>';
