...
class Block extends AbstractPanel
{
	...
	public function getEditChildHtml($block, $child) {
        ...
        $html .= '<li><a href="' . $this->getUrl('mpanel/create/element', array('block' => $block, 'id' => $child->getId(), 'type' => $child->getType())) . '" class="popup-link" title="' . __('Edit') . '"><em class="fa fa-edit">&nbsp;</em></a></li>';
	...
	public function getContentOfBlock($block){
		...
		/* switch ($block->getType()) {
            case 'static':
                return $this->getLayout()->createBlock('Magento\Cms\Model\Block')->setBlockId($block->getStaticBlockId())->toHtml();
                break;
            default:
